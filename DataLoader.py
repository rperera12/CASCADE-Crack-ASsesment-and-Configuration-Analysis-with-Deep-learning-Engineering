...
class CustomCollate:
    def __call__(self, batch):
        x = torch.stack([torch.from_numpy(item[0]) for item in batch]).squeeze()
        y = torch.cat([torch.from_numpy(item[1]) for item in batch], dim=0)
        return x, y
...
class DataLoader(torch.utils.data.Dataset):
    ...
    def normalize_data(self, data):
        # Using Min-Max normalization to scale the data between 0 and 1
        scaler = MinMaxScaler()
    ...
    def __getitem__(self, index):
        ...
        if self.normalize:
            # Normalizing x (potential energy) and y (crack tip positions) if required
