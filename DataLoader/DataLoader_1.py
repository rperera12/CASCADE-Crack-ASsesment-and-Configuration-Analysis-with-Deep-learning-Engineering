...
class CustomCollate:
    def __call__(self, batch):
        ...
        iteration_names = [item[2] for item in batch]
        return x, y, iteration_names

class DataLoader_X(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        iteration_name = f'var_iter_{current_iter}'  # Get the current iteration name
        ...
        return self.pad_tensor(x.flatten()), y.flatten(), iteration_name
...
if __name__ == "__main__":
    ...
    for i, (x, y, iteration_name) in enumerate(dataloader):  # We now also get the iteration name
        print(f"Iteration {i+1}:")
        print(f"Iteration Name: {iteration_name}")  # Print the iteration name
    ...
    for i, (x, y, iteration_names) in enumerate(dataloader):
